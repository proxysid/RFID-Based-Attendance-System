...
byte cards[numOfCards][4] = {{0x13, 0x5D, 0x57, 0xFB}}; // array of UIDs of rfid cards
...
String names[numOfCards] = {"Siddharth"};//student names
long sNumbers[numOfCards] = {16242};//student sNumbers
...
void setup() {
  ...
  if (! rtc.begin()) {
    ...
    while (1);
      ...
      rtc.adjust(DateTime(F(_DATE), F(TIME_)));//time will set auto when it compiles
